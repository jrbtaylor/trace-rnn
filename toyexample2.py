...
if not online_training:
    ...
    # scan
    y0 = T.zeros((n_out,),dtype=theano.config.floatX)
    [output,cost,Ao,Bo], updates = theano.scan(fn=step,
                                  sequences=[x.dimshuffle([1,0,2]),
                                             y.dimshuffle([1,0,2])],
                                  outputs_info=[T.alloc(y0,x.shape[0],n_out),
                                                None,A,B],
...
#       meaning that using T.grad() within the step function is limiting it to 0-step backprop
    ...
    batch_size = 100
    A_val = rng.uniform(low=0,high=1,size=(n_in,n_out)).astype('float32')
    B_val = rng.uniform(low=0,high=1,size=(n_out,n_out)).astype('float32')
    index = T.lscalar()  # index to a [mini]batch
    train_step = theano.function(inputs=[index,A,B],
                                 outputs=(output,T.mean(cost),Ao[-1],Bo[-1]),
                                 updates=updates,
                                 givens={x:x_train[index*batch_size:(index+1)*batch_size],
                                         y:y_train[index*batch_size:(index+1)*batch_size]})
    ...
    for epoch in range(n_epochs):
        train_loss = 0
        for batch in range(n_batches):
            output,batch_loss,A_val,B_val = train_step(batch,A_val,B_val)
            train_loss += batch_loss
            if numpy.any(numpy.isnan(output)):
                break
        print('Epoch-%d loss: %f' % (epoch,train_loss))
