...
class rnn_dni_trace(object):
    def __init__(self,n_in,n_hidden,n_out,steps,rng=rng):
        ...
        # note: traces for biases are always 1, no activations to track
        self.traces = []
        for W in [self.Wx,self.Wh,self.Wy]:
            trace = theano.shared(
                            numpy.zeros(param.get_value(borrow=True).shape,
                                        dtype=theano.config.floatX))
            self.traces.append(trace)
        for b in [self.bh,self.by]:
            trace = theano.shared(
                            numpy.ones(param.get_value(borrow=True).shape,
                                       dtype=theano.config.floatX))
            self.traces.append(trace)
